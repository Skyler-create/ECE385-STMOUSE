...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	uint32_t temporary;
	    ...
	    temporary = hdmi_ctrl -> palette[color / 2];
	    if (color % 2) {
	        temporary &= 0xFE001FFF;
	        temporary |= (red << 21) | (green << 17) | (blue << 13);
	    } else {
	        temporary &= 0xFFFFE001;
	        temporary |= (red <<  9) | (green <<  5) | (blue <<  1);
	    }
	    hdmi_ctrl->palette[color / 2] = temporary;
}
